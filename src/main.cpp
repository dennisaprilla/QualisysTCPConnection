...
int main()
{
	...
	// Qualisys is the one who control every device to start. So the situation become quite tricky. There is an elegant
	...
	// 
	// Since the qualisys thread will be created inside the IF block, so we need to declare the thread (without doing anything)
	...
	//
	// So, i create a this one line block here, instead of in the else{} block from if(useQualisys) below. Everything is
